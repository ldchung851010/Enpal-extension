...
test('updates an exact range with RAW values', async () => {
  ...
  await client.updateValues('sheet/with space', 'ACTIVE!A1:B1', [['READY', '1']]);

  assert.match(calls[0].url, /sheet%2Fwith%20space\/values\/ACTIVE!A1%3AB1\?valueInputOption=RAW$/);
  assert.equal(calls[0].options.method, 'PUT');
  assert.equal(calls[0].options.body, JSON.stringify({ values: [['READY', '1']] }));
