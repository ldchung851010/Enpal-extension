...
export function createSessionRepository({
  ...
  function assertWorkspaceRows(headers, rawRows) {
    if (!headers.includes('workspace_id')) return;
    const foreign = rawRows.find(
      row => row.workspace_id && row.workspace_id !== config.id
    );
    if (foreign) {
      throw new EnpalError(
        ERROR_CODES.CONSISTENCY_ERROR,
        'Sessions sheet contains foreign workspace_id: ' + foreign.workspace_id,
        false
      );
    }
  }
  ...
  async function readTable() {
    ...
    assertWorkspaceRows(headers, rawRows);
    return {
      values,
      headers,
      rawRows,
  ...
  function requireColumn(headers, name) {
    ...
    if (index < 0) {
      throw new Error('Sessions sheet missing required column: ' + name);
    }
    return index;
  }
  ...
  return {
    ...
    async listActive() {
      ...
      if (active.length > 1) {
        throw new EnpalError(
          ERROR_CODES.CONSISTENCY_ERROR,
          'More than one durable active session found in Workspace ' +
            config.id + ': ' +
    ...
    async createStartingSession(session) {
      ...
      const statusField = fieldName(
        table.headers,
        'lifecycle_status',
        'status'
      );
      if (!statusField) {
        throw new Error('Sessions sheet missing required status column');
      }
      ...
      if (active.length > 0) {
        throw new EnpalError(
          ERROR_CODES.CONSISTENCY_ERROR,
          'Cannot create STARTING session while ' +
            active[0].session_id +
            ' is active in Workspace ' +
            config.id,
          false
        );
      }
      ...
      if (table.headers.includes('workspace_id')) {
        row.workspace_id = config.id;
      }
    ...
    async bindChat(sessionId, chatUrl) {
      ...
      if (current !== '') {
        throw new Error(
          'Session ' + sessionId + ' is already bound to ' + current
        );
      }
    ...
    async markState(sessionId, status, patch = {}) {
      ...
      if (table.headers.includes('workspace_id')) {
        nextRaw.workspace_id = config.id;
      }
