...
function normalizeLesson(row) {
  ...
  return {
    ...row,
    curriculum_sequence: Number.isFinite(sequence) ? sequence : row.curriculum_sequence
  };
}
...
export function createCurriculumRepository({
  ...
  async function readAll() {
    return rowsToObjects(await sheets.getValues(spreadsheetId, range)).map(normalizeLesson);
  }
  ...
  return {
    async getLesson(sequence) {
      ...
      return rows.find((row) => Number(row.curriculum_sequence) === wanted) ?? null;
    },
    ...
    async getNextLesson(completedSequences = []) {
      ...
      const candidates = rows
        ...
        .sort((a, b) => Number(a.curriculum_sequence) - Number(b.curriculum_sequence));
